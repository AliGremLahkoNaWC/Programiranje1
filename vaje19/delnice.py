...
def preberi(ime_datoteke):
    '''vrne par seznamov (datumov in vrednost delnic na ta dan)'''
    datumi = []
    vrednosti = []
    for vrstica in open(ime_datoteke, 'r'):
        vrstica = vrstica[:-1]
        vrstica =  vrstica.split(',')
        datumi.append(str(vrstica[0]).replace(' ',''))
        vrednosti.append(vrstica[1])
...
# Sestavite funkcijo
# `povratek(imeDat)`, ki kot parameter sprejme ime datoteke z vrednostmi delnice
...
import math
def povratek(imeDat):
    '''funkcija vrne seznam logaritemskih povratkov'''
    stev = 0
    tab = []
    prej = 0
    podatki = preberi(imeDat)
    ...
    for vrstica in podatki[1]:
        ...
        if stev % 2 == 0:
            prej = int(vrstica)
        if stev % 2 != 0:
            loga = math.log(1 + prej) + math.log(1 + int(vrstica))
            tab.append(loga)
    if stev == 1:
        return []
    return tab
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjozMzY3LCJwYXJ0IjoxOTQ3MX0:1hC2av:1eYqrRP7CNDZ_Tu4MM-C8MIb97A'
        try:
            _d = """2012-01-01,53.05
            2012-01-02,54.08
            2012-01-03,58.38
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjozMzY3LCJwYXJ0IjoxOTQ3Mn0:1hC2av:xi-RvSVWoUF2xpBFGb-OFMZXblg'
        try:
            _d = """2012-01-01,53.05
            2012-01-02,54.08
            2012-01-03,58.38
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjozMzY3LCJwYXJ0IjoxOTQ3M30:1hC2av:Ck5kQJxM2ubnm-rqegVaZ9wFric'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjozMzY3LCJwYXJ0IjoxOTQ3NH0:1hC2av:HnIs40mXPC3qQFDvuEmo_OCHlao'
        try:
