...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMDAxOCwidXNlciI6MzM2N30:1h1nym:AQcE7RxkAFuLfGEL_kaidbI_0iM'
        try:
            ...
            Check.secret(slovar_kock([]))
            Check.secret(slovar_kock(['2x2.rdeča', '2x2.rdeča', 'nogice.modra', '2x2.modra']))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMDAxOSwidXNlciI6MzM2N30:1h1nym:z6gtihAn91sDKpj2t7-z6ZUdPxE'
        try:
            model = {'4x1': {'modra': 1}, '2x2': {'rdeča': 2, 'modra': 1}}
            skatla = {'4x1': {'modra': 3, 'rdeča': 2}, '2x2': {'rdeča': 4, 'modra': 1, 'rumena': 5}}
            model_2 = {'4x1': {'rumena': 1}, '2x2': {'rdeča': 1, 'rumena': 2}}
            ...
            Check.secret(lahko_sestavimo(skatla, model))
            Check.secret(lahko_sestavimo(skatla, model_2))
            Check.secret(lahko_sestavimo({}, {}))
            Check.secret(lahko_sestavimo(dict(), skatla_2))
            Check.secret(lahko_sestavimo(skatla, skatla_2))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMDAyMCwidXNlciI6MzM2N30:1h1nym:vkcGFuGELktg7NsauTDBLVM1Qbs'
        try:
            ...
            Check.secret(hitro_sestavljanje(skatla_4, model_4))
            Check.secret(hitro_sestavljanje(skatla_5, model_4))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
